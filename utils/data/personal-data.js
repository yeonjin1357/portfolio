export const personalData = {
  name: '최연진',
  profile: '/profile.png',
  designation: '프론트엔드 개발자',
  description: `안녕하세요, 사용자 중심의 UI/UX 구현에 강점을 가진
    웹 퍼블리셔 출신 프론트엔드 개발자 최연진(JINJIN)입니다.

    2년간의 퍼블리싱 경험을 바탕으로, 웹 표준과 접근성을 고려한 마크업에 능숙하며,
    최근 1년간은 Vue3를 중심으로 한 프론트엔드 개발 업무를 수행하며
    웹 애플리케이션 구축 경험을 쌓았습니다.
    
    제 포트폴리오를 통해 다양한 프로젝트와 경험을 확인해 보시고,
    함께 성장하며 흥미로운 웹 경험을 만들어가고 싶은 분들과의 협업을 기대합니다.
    감사합니다! ✌`,
  email: 'yeonjin19980125@gmail.com',
  phone: '010-2798-7556',
  github: 'https://github.com/yeonjin1357',
  notion: 'https://www.notion.so/jinjin98/13e27eec4b9d437180818e50846d1584',
  velog: 'https://velog.io/@yeonjin1357',
  linkedIn: 'https://www.linkedin.com/in/연진-최-004ba62ab/',
  resume:
    'https://drive.google.com/file/d/1-Uo0rPAgx8JYzLxoLYbB9CbAHbODTCZZ/view?usp=drive_link',
};
